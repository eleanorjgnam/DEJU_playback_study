...
```{r}
Time_Within_10_Meters = deju_data |>
  group_by(Bands, Treatment) |>
  ...
  summarise(Time_Within_10_Meters = sum(Time_Spent, na.rm = TRUE), .groups = "drop")
```

Number of Songs Within 10m

```{r}
Number_Of_Songs_Within_110_Meters = deju_data |>
  group_by(Bands, Treatment) |>
  filter(Estimated_Distance_of_Focal_Male < 10) |>
  filter(str_detect(Annotation, "Focalsong")) |>
  summarise(Number_Of_Songs_While_Within_10_Meters = length(Annotation), .groups = "drop")
```
